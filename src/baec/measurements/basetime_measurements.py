...
class ProjectsIDs:
    ...
    def __init__(self, credentials: str | PathLike[str] | ReadCsvBuffer[bytes] | ReadCsvBuffer[str], s3bucket: str = 'baec'):
        ...
        try:
            ...
            raise ValueError('The AWS Access Key ID or Access Key Password you provided does not exist in our records.')
    ...
    def make_SettlementRodMeasurementSeries(self, company: str, project: str, rod_id: str):
        ...
        if company in self.dic_projects and project in self.dic_projects[company] and rod_id in self.dic_projects[company][project]:
                        ...
                        for measurement in json_dict['Measurements']:
                            if measurement['Point ID'] == rod_id:
                                ...
                                if measurement['Comments Project'] == 'No comment':
                                    ...
                                    for error_code in error_integer_list:
                                        if self.dict_errors[error_code]['status message level'] == 'INFO':
                                            status_messages.append(StatusMessage(
                                                code=error_code,
                                                description=self.dict_errors[error_code]['description'],
                                                level=StatusMessageLevel.INFO
                                            ))
                                        elif self.dict_errors[error_code]['status message level'] == 'WARNING':
                                            status_messages.append(StatusMessage(
                                                code=error_code,
                                                description=self.dict_errors[error_code]['description'],
                                                level=StatusMessageLevel.WARNING
                                            ))
                                        elif self.dict_errors[error_code]['status message level'] == 'ERROR':
                                            status_messages.append(StatusMessage(
                                                code=error_code,
                                                description=self.dict_errors[error_code]['description'],
                                ...
                                test_measurement = SettlementRodMeasurement(
                                    ...
                                    rod_top_x=measurement['Coordinates ARP']['X'] or float('nan'),
                                    rod_top_y=measurement['Coordinates ARP']['Y'] or float('nan'),
                                    rod_top_z=measurement['Coordinates ARP']['Z'] or float('nan'),
                                    rod_length=measurement['Vertical Offset (Meter)'] or float('nan'),
                                    rod_bottom_z=measurement['Height ground level']['Zuncorrected'] or float('nan'),
                                    ground_surface_z=measurement['Coordinates ARP']['Soil level'] or float('nan'),
            ...
            raise ValueError(f"{company} is in the user list and {project} is in the project list, but not rod_id: {rod_id}")
        elif company in self.dic_projects:
            raise ValueError(f"{company} is in the user list, but not the project: {project}")
        else:
    ...
    @staticmethod
    def convert_epsg_string_to_list_int(epsg_string: str):
        """
